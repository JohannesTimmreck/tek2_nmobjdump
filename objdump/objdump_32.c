...
static void print_linestart(uint32_t line_start, uint32_t max_size, char *str)
{
    int loops = 0;
...
static void print_last_spaces(uint32_t i)
{
    for (; i % 16 != 0;) {
        ++i;
        printf(" ");
...
static void print_section(Elf32_Shdr *section, void *buffer)
{
...
void apply_32_s_flag(Elf32_Ehdr *ehdr)
{
    Elf32_Shdr *sections = (Elf32_Shdr *)((char *)ehdr + ehdr->e_shoff);
...
{
    apply_32_f_flag(ehdr);
    printf("\n");
    apply_32_s_flag(ehdr);
}